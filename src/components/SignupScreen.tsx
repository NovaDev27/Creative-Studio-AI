...
import { Coffee, Sparkles, AlertCircle, Loader2, UserPlus } from 'lucide-react';
...
const SignupScreen: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-creme dark:bg-ink font-sans relative overflow-hidden transition-colors duration-500">
      {/* Background elements */}
      <div className="absolute top-[-10%] left-[-5%] w-64 h-64 bg-sakura/10 rounded-full blur-3xl animate-pulse" />
      <div className="absolute bottom-[-10%] right-[-5%] w-96 h-96 bg-caramel/10 rounded-full blur-3xl animate-pulse" />
      ...
      >
        <div className="flex justify-center mb-8">
          ...
          >
            <UserPlus className="w-8 h-8 text-creme dark:text-ink" />
          </motion.div>
        </div>
        ...
        <form onSubmit={handleSignup} className="space-y-5">
          <div className="space-y-1">
            <label htmlFor="fullName" className="block text-xs font-bold uppercase tracking-widest text-coffee/70 dark:text-creme/70 ml-1">
              Nome Completo
            </label>
            <input
              type="text"
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            type="submit"
            className="w-full flex justify-center items-center py-4 px-6 bg-mossa dark:bg-matcha text-creme dark:text-ink rounded-2xl font-bold uppercase tracking-widest text-xs hover:bg-umi dark:hover:bg-creme transition-all shadow-xl disabled:opacity-70 disabled:cursor-not-allowed"
            disabled={loading}
          >
            {loading ? (
          ...
          <motion.button
            ...
            className="w-full flex justify-center items-center py-4 px-6 bg-white dark:bg-white/10 border border-snow dark:border-white/10 text-ink dark:text-creme rounded-2xl font-bold uppercase tracking-widest text-xs hover:bg-creme dark:hover:bg-white/20 transition-all shadow-sm"
          >
        ...
        <div className="mt-10 pt-8 border-t border-snow dark:border-white/10 text-center">
          <p className="text-sm text-ink/60 dark:text-creme/60">
            Já possui um refúgio?{' '}
            <Link to="/login" className="font-bold text-mossa dark:text-matcha hover:text-umi dark:hover:text-creme transition-colors">
              Entre aqui
            </Link>
          </p>
        </div>
      ...
      <div className="absolute bottom-1/4 right-10 opacity-20 pointer-events-none">
        <Coffee className="w-12 h-12 text-coffee dark:text-sakura animate-pulse" />
      </div>
    </div>
  );
};
